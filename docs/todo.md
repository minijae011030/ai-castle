...
- [ ] 선택 TODO 협상 UX (MVP)
  - Todo 목록에서 단일/다중 선택 + `협상 요청` 버튼
  - 협상 요청 모달: 기본 문구(예: "오늘 너무 힘들어요") + 자유 입력
...
- [ ] 메인 에이전트 채팅 UI
  ...
  - 협상 발화 시 선택 Todo IDs를 포함해 협상 API로 연결
...
- [ ] 양방향 협상 (선택 TODO 기반 NEGOTIATE 흐름)
...
- [ ] 협상 상태/이력 모델 추가
  ...
  - 협상 이력 저장: selected_todo_ids, user_reason, proposal_json, selected_option, status
...
1) Front: Todo 다중 선택 + 협상 요청 모달 + 제안 카드/적용 UI  
