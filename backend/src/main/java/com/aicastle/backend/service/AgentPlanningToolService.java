...
public class AgentPlanningToolService {
  ...
  public List<CalendarEventItem> getCalendarEvents(
    ...
    log.info("👉 [에이전트 툴:일정조회] 사용자ID={}, 조회범위={}~{}", userId, startDate, endDate);
    ...
    log.info("👉 [에이전트 툴:일정조회] 원본건수={}, 병합건수={}", all.size(), events.size());
    return events;
  }
  ...
  public List<TodoItem> getTodos(
    ...
    log.info("👉 [에이전트 툴:할일조회] 원본건수={}, 반환건수={}", todos.size(), result.size());
    return result;
  }
  ...
  public List<TodoItem> getTodosByAgent(
    ...
    log.info(
        "👉 [에이전트 툴:에이전트할일조회] 사용자ID={}, 에이전트ID={}, 조회범위={}~{}, 완료필터={}",
        userId,
        agentId,
        startDate,
        endDate,
    ...
    log.info("👉 [에이전트 툴:에이전트할일조회] 원본건수={}, 반환건수={}", todos.size(), result.size());
    return result;
  }
  ...
  public UserConstraintResponse getUserConstraints(Long userId) {
    ...
    log.info(
        "👉 [에이전트 툴:사용자제약조회] 사용자ID={}, 시작시간={}, 종료시간={}, 강도={}",
        userId,
  ...
  public ReschedulePlanResponse simulateReschedule(Long userId, RescheduleSimulateRequest request) {
    ...
    log.info(
        "👉 [에이전트 툴:재배치시뮬레이션] 사용자ID={}, 조회범위={}~{}, 대상할일ID={}, 최대이동일수={}, 최소버퍼분={}",
        userId,
        request.startDate(),
        request.endDate(),
        request.todoIds(),
    ...
    log.info(
        "👉 [에이전트 툴:재배치시뮬레이션] 대상건수={}, 계획건수={}, 충돌건수={}, 실행가능={}",
        targetTodos.size(),
        items.size(),
        conflicts.size(),
        feasible);
  ...
  public ReschedulePlanResponse validatePlan(List<ReschedulePlanItem> planItems, Long userId) {
    if (planItems == null || planItems.isEmpty()) {
      log.info("👉 [에이전트 툴:계획검증] 사용자ID={}, 입력계획없음", userId);
      return new ReschedulePlanResponse(UUID.randomUUID().toString(), true, List.of(), List.of());
    }
    log.info("👉 [에이전트 툴:계획검증] 사용자ID={}, 입력계획건수={}", userId, planItems.size());
    ...
    boolean feasible = conflicts.isEmpty();
    log.info("👉 [에이전트 툴:계획검증] 충돌건수={}, 실행가능={}", conflicts.size(), feasible);
  ...
  public RescheduleApplyResponse applyPlan(Long userId, RescheduleApplyRequest request) {
    if (request.items() == null || request.items().isEmpty()) {
      log.info("👉 [에이전트 툴:재배치적용] 사용자ID={}, 적용모드={}, 입력항목없음", userId, request.mode());
      return new RescheduleApplyResponse(
          UUID.randomUUID().toString(), request.mode(), 0, List.of());
    }
    log.info(
        "👉 [에이전트 툴:재배치적용] 사용자ID={}, 적용모드={}, 입력항목수={}",
        userId,
        request.mode(),
        request.items().size());

    if (request.mode() == ApplyMode.DRAFT) {
      log.info("👉 [에이전트 툴:재배치적용] 초안모드로 저장, DB 반영 없음");
      return new RescheduleApplyResponse(
    ...
    log.info("👉 [에이전트 툴:재배치적용] 커밋 완료, 적용건수={}", appliedItems.size());
    return new RescheduleApplyResponse(
  ...
  public RescheduleRollbackResponse rollback(Long userId, RescheduleRollbackRequest request) {
    if (request.appliedItems() == null || request.appliedItems().isEmpty()) {
      log.info("👉 [에이전트 툴:되돌리기] 사용자ID={}, 되돌릴항목없음", userId);
      return new RescheduleRollbackResponse(UUID.randomUUID().toString(), 0);
    }
    log.info("👉 [에이전트 툴:되돌리기] 사용자ID={}, 입력적용항목수={}", userId, request.appliedItems().size());
    ...
    scheduleOccurrenceRepository.saveAll(todoById.values());
    log.info("👉 [에이전트 툴:되돌리기] 되돌린건수={}", count);
    ...
    String explanation =
        "요청 메시지(\""
            ...
            + (conflictCount > 0
                ...
                : "충돌 없이 적용 가능한 계획입니다.");
    log.info(
        "👉 [에이전트 툴:설명생성] 이동건수={}, 유지건수={}, 충돌건수={}", movedCount, unchangedCount, conflictCount);
    return explanation;
  }
  ...
  public TimePreferenceResponse resolveTimePreference(Long userId, TimePreferenceRequest request) {
    ...
    String reason =
        "dayStart="
            ...
            + ", sleepDebtMinutes="
            + sleepDebtMinutes;
    log.info("👉 [에이전트 툴:시간선호해결] 사용자ID={}, 추천시간대={}", userId, recommendedWindow);
  ...
  public EnergyCurveResponse getEnergyCurve(Long userId, EnergyCurveRequest request) {
    ...
    log.info("👉 [에이전트 툴:에너지곡선] 사용자ID={}, 일수={}", userId, items.size());
  ...
  public TaskEffortEstimateResponse estimateTaskEffort(TaskEffortEstimateRequest request) {
    log.info("👉 [에이전트 툴:작업시간추정] 요청={}", request);
    ...
    log.info("👉 [에이전트 툴:작업시간추정] 결과={}", response);
    return response;
  }

  public SplitTaskResponse splitTask(SplitTaskRequest request) {
    log.info("👉 [에이전트 툴:작업분할] 요청={}", request);
    int total =
    ...
    log.info("👉 [에이전트 툴:작업분할] 분할건수={}", items.size());
    return response;
  }
  ...
  public PriorityRankResponse rankTaskPriority(PriorityRankRequest request) {
    ...
    log.info("👉 [에이전트 툴:우선순위정렬] 정렬결과건수={}", items.size());
    return response;
  }
  ...
  public BufferInsertResponse insertBufferBlocks(BufferInsertRequest request) {
    log.info("👉 [에이전트 툴:버퍼삽입] 요청={}", request);
    int bufferMinutes =
  ...
  public ApplySafeguardResponse applyWithSafeguard(Long userId, ApplySafeguardRequest request) {
    ...
    if (request != null && request.dryRun()) {
      ...
      log.info("👉 [에이전트 툴:안전적용] 드라이런 영향건수={}", response.impactedCount());
      return response;
    }
    ...
    log.info("👉 [에이전트 툴:안전적용] 커밋 영향건수={}", response.impactedCount());
    return response;
  }

  public String explainPlanBrief(ExplainBriefRequest request) {
    log.info("👉 [에이전트 툴:요약설명] 요청={}", request);
    ...
    log.info("👉 [에이전트 툴:요약설명] 결과={}", response);
    return response;
  }
